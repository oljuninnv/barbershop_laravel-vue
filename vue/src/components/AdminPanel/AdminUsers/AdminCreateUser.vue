<template>
  <div class="flex min-h-full flex-1 flex-col justify-center px-6 py-2 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <form class="max-w-lg mx-auto p-4 bg-white shadow-md rounded flex flex-col gap-5" @submit.prevent="addUser" enctype="multipart/form-data">
        <div>
        ...
        <div>
          ...
          <div class="mt-2">
            <input id="login" name="login" type="text" v-model="formData.login" required class="input_text" />
          </div>
        </div>
        ...
        <div class="flex items-center mb-4">
          <input type="checkbox" id="add_to_worker" v-model="isWorker" class="mr-2" />
...
const isWorker = ref(false); // Реактивная переменная для чекбокса
...
async function addUser() {
  try {
    ...
    formData.value.append('is_worker', isWorker.value); // Добавляем значение чекбокса

    errors.value = [];
    
    // Отправка данных через axios.post
    const response = await axios.post('/api/add_user', formData.value, {
      headers: {
    ...
    isWorker.value = false; // Сбрасываем состояние чекбокса
  } catch (error) {
    console.log(error);
    if (error.response) {
